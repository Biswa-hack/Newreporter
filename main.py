...
RECIPIENTS_FILE = "recipients.txt"  # File containing one email per line
...
def analyze_article(article):
    ...
    prompt = f"CONTEXT: {article.get('description', '')}\nTITLE: {article.get('title')}\nOutput HTML format only. Use <p> and <strong> tags. Identify: 1. Core Impact, 2. Sentiment."
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; background: #f4f4f4; padding: 20px;">
        <div style="max-width: 600px; margin: auto; background: #fff; padding: 30px; border-top: 8px solid #b71c1c;">
            ...
            <div style="background: #fdf2f2; padding: 15px; border-radius: 5px; margin: 20px 0;">
                <h3 style="margin-top:0; color:#b71c1c;">📊 Market & Commodities</h3>
                <table width="100%" style="font-size: 14px; border-collapse: collapse;">{market_rows}</table>
            </div>
            <div style="margin-top: 30px;">{analysis_html}</div>
            <div style="font-size: 10px; color: #aaa; text-align: center; margin-top: 40px;">Generated by AI Intelligence Bot v3.0 | 2026 Edition</div>
        </div>
    </body>
    </html>
    """

# --- 6. DISPATCH (MULTIPLE RECIPIENTS) ---
def run_report():
    ...
    with open(RECIPIENTS_FILE, "r") as f:
        recipient_list = [line.strip() for line in f if line.strip() and "@" in line]
    ...
    for art in articles:
        ...
        full_analysis += f"<h3>{art['title']}</h3>{analyze_article(art)}<hr style='border:0; border-top:1px solid #eee;'>"
    ...
    try:
        ...
        print(f"\n✨ All {len(recipient_list)} reports dispatched successfully!")
    except Exception as e:
